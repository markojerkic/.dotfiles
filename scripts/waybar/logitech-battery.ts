...
for (const deviceId of [1, 2]) {
  try {
    ...
    // Solaar command failed, skip this device
    continue;
  }
}
...
function isBatteryCritical(device: Device | undefined) {
  ...
  return (
    device.status === "DISCHARGING" && Number.parseFloat(device.battery) <= 30
  );
}
...
function isBatteryWarning(device: Device | undefined) {
  ...
  return (
    device.status === "DISCHARGING" && Number.parseFloat(device.battery) <= 50
  );
}
...
const result = {
  ...
  class: className,
  tooltip: device?.status === "DISCHARGING" ? "󰁾 Discharging" : "󰂅 Recharging",
};
